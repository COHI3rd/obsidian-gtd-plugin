...
- 🖱️ **右クリックメニュー** - タスクを右クリックでステータス変更・プロジェクト割り当て
  - ステータス変更（Inbox, Next Action, Today, Waiting, Someday, Trash）
  - プロジェクト割り当て（アクティブなプロジェクトのみ表示）
  - プロジェクトから外す
  - リアルタイム更新 - 新規プロジェクト作成後すぐにメニューに反映
